...
%
%  sp_to_vtk (u, space, geometry, npts, filename, epsilon_coeffs, fieldnames, [option], [lambda_lame, mu_lame])
%
% INPUT:
%     
...
%     epsilon_coeffs:   coefficent vector with patch material functions
...
function sp_to_vtk_epsilonScaled (u, space, geometry, npts, epsilon_coeffs, filename, fieldname, varargin)
...
'</VTKFile> \n');
  ...
  for iptc = 1:space.npatch
    ...
    if (isempty (space.dofs_ornt))
      sp_to_vtk (epsilon_coeffs(iptc) * u(space.gnum{iptc}), space.sp_patch{iptc}, geometry(iptc), npts, ...
                           filename_patch, fieldname, varargin{:})
    else
      sp_to_vtk (epsilon_coeffs(iptc) * u(space.gnum{iptc}) .* space.dofs_ornt{iptc}', space.sp_patch{iptc}, geometry(iptc), npts, ...
                           filename_patch, fieldname, varargin{:})
    end
  end
  fprintf (fid, str3);

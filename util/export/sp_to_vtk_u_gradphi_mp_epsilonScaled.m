...
function sp_to_vtk_u_gradphi_mp_epsilonScaled (u, space_u, phi, space_phi, geometry, npts, filename, epsilon_coeffs, fieldname, varargin)
...
'</VTKFile> \n');
  ...
  for iptc = 1:space_u.npatch
    ...
    if (isempty (space_u.dofs_ornt))
      sp_to_vtk_u_gradphi (epsilon_coeffs(iptc) * u(space_u.gnum{iptc}), space_u.sp_patch{iptc}, epsilon_coeffs(iptc) * phi(space_phi.gnum{iptc}), space_phi.sp_patch{iptc}, geometry(iptc), npts, ...
                           filename_patch, fieldname, varargin{:})
    else
      sp_to_vtk_u_gradphi (epsilon_coeffs(iptc) * u(space_u.gnum{iptc}) .* space_u.dofs_ornt{iptc}', space_u.sp_patch{iptc}, epsilon_coeffs(iptc) * phi(space_phi.gnum{iptc}), space_phi.sp_patch{iptc}, geometry(iptc), npts, ...
                           filename_patch, fieldname, varargin{:})
    end
  end
  fprintf (fid, str3);
